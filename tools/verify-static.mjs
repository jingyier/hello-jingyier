...
const music = read("music/index.html");
const homeScript = readFileSync(join(root, "src/scripts/home.js"), "utf8");
const musicScript = readFileSync(join(root, "src/scripts/music.js"), "utf8");
const homePage = readFileSync(join(root, "src/pages/index.astro"), "utf8");
const musicPage = readFileSync(join(root, "src/pages/music.astro"), "utf8");
const messageFunction = readFileSync(join(root, "functions/api/messages.ts"), "utf8");
const musicFunction = readFileSync(join(root, "functions/api/music.ts"), "utf8");
...
const musicData = JSON.parse(readFileSync(join(root, "src/content/music.json"), "utf8"));
...
assert("core pages are generated", Boolean(index && about && garden && notes && work && music));
...
assert("home page has reviewed message containers and honeypot", homePage.includes("message-board-card") && homePage.includes("data-message-approved-list") && homePage.includes("data-message-local-list") && homePage.includes("message-honeypot") && homePage.includes("data-home-page"));
